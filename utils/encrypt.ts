...
export function encrypt(text: any) {
  ...
  return {
    iv: iv.toString(REPRESENTATION_TYPE),
    encryptedData: encrypted.toString(REPRESENTATION_TYPE),
  };
}
...
function pkEncryptionKey(
  userId: string,
  tokenName: string,
  network: string,
  address: string
) {
  const addressMD5: string = crypto
    .createHash('md5')
    .update(address)
    .digest('hex');
  let keyMD5: string;
  if (tokenName) {
    keyMD5 = crypto
      .createHash('md5')
      .update(userId + tokenName + network + addressMD5)
      .digest('hex');
  } else {
    keyMD5 = crypto
      .createHash('md5')
      .update(userId + network + addressMD5)
      .digest('hex');
  }
  return keyMD5;
}

export function encryptPK(
  userId: string,
  tokenName: string,
  network: string,
  address: string,
  ...
  return encryptWithKey(
    privateKey,
    pkEncryptionKey(userId, tokenName, network, address)
  );
}
...
export function decryptPK(
  ...
  console.log('decryptPK', { userId, tokenName, network, address, privateKey });
  return decryptWithKey(
    concatenatedToIVPair(privateKey),
    pkEncryptionKey(userId, tokenName, network, address)
  );
}
